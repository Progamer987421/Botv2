...
// POST /proxy/custom — single custom proxy, no Webshare key needed
// Body: { proxy: "socks5://user:pass@host:port" }  or  { proxy: "host:port:user:pass" }
app.post('/proxy/custom', auth, async (req, res) => {
  const { proxy } = req.body;
  if (!proxy) return res.status(400).json({ error: 'proxy string required' });

  if (uiProxyManager) uiProxyManager.stop();

  try {
    uiProxyManager = ProxyManager.fromSingle(proxy);
    await uiProxyManager.init();
    manager.proxyManager = uiProxyManager;
    manager._proxyReady  = Promise.resolve();
    const p = uiProxyManager.verified[0];
    res.json({ ok: true, host: p.host, port: p.port, latency: p.latency });
  } catch (err) {
